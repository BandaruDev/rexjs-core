...
function hook(compiler, action) {
    compiler.hooks.compilation.tap('cleancss-webpack-plugin', function (compilation) {
        compilation.hooks.optimizeChunkAssets.tapPromise('cleancss-webpack-plugin', function (chunks) { return action(compilation, chunks); });
    });
}
            ...
            var actions = files
                ...
                return Promise.resolve()
                    ...
                    if (output.errors && output.errors.length > 0) {
                        output.errors
                            .forEach(function (error) { return compilation.errors.push(new Error(error)); });
                        return;
                    }
